...
var sValues = [86, 78, 70, 200, 93, 0, 20, 30, 0, true, 0];
...
function init()
{
	...
	coverflow.addListener(FWDRoyal3DCoverflow.INTRO_START, onIntroStart);
	coverflow.addListener(FWDRoyal3DCoverflow.INTRO_FINISH, onIntroFinish);
	coverflow.addListener(FWDRoyal3DCoverflow.DATA_LOADED, onDataLoaded);
...
//####################################//
function setupMenu(){
	FWDR3DCovPageMenu.setPrototype();
	pageMenu_do = new FWDR3DCovPageMenu({
		disabledButton:0,
	...
	pageMenu_do.addListener(FWDR3DCovPageMenuButton.CLICK, buttonClickHandler);
}
...
function setupSliders(){
	FWDR3DCovSlidersMenu.setPrototype();
	slidersMenu_do = new FWDR3DCovSlidersMenu(slidersHolder_el, sValues, coverflow.data);
	
	slidersMenu_do.addListener(FWDR3DCovSlidersMenu.CHANGE, onSlidersMenuChange);
...
{
	FWDR3DCovPageThumbs.setPrototype();
	
	pageThumbs_do = new FWDR3DCovPageThumbs({
	...
	pageThumbs_do.addListener(FWDR3DCovPageThumb.CLICK, onThumbPressedHandler);
}
...
function onThumbPressedHandler(e)
{
	...
	if(e.id == 0){
		...
		coverflowHeight = 538;
		setupCoverflow();
		sValues = [86, 78, 70, 200, 93, 0, 20, 30, 0, true, 0];
	}else if(e.id == 1){
		body_el.appendChild(playlist1SingleCat_el);
		coverflowHeight = 538;
		setupCoverflow1();
		sValues = [80, 80, 67, 152, 145, 3, 20, 30, 3, true, 0];
	}else if(e.id == 2){
		body_el.appendChild(playlist2_el);
		coverflowHeight = 538;
		setupCoverflow2();
		sValues = [85, 81, 61, 142, 88, 0, 20, 30, 0, true, 0];
	}else if(e.id == 3){
		body_el.appendChild(playlist2SingleCat_el);
		coverflowHeight = 538;
		setupCoverflow3();
		sValues = [80, 128, 70, 225, 154, 3, 20, 30, 3, true, 0];
	}
...
function setupCoverflow1(){
	coverflow = new FWDRoyal3DCoverflow({
		...
		thumbnailZOffset3D:152,
		thumbnailZSpace3D:145,
		thumbnailYAngle3D:67,
...
function setupCoverflow2(){
	
	coverflow = new FWDRoyal3DCoverflow({		
		//required settings
		coverflowHolderDivId:"myDiv",
		coverflowDataListDivId:"playList3",
...
function setupCoverflow3(){
	
	coverflow = new FWDRoyal3DCoverflow({		
		//required settings
		coverflowHolderDivId:"myDiv",
		coverflowDataListDivId:"playList4SingleCat",
		displayType:"responsive",
		autoScale:"yes",
		coverflowWidth:940,
		coverflowHeight:538,
		...
		thumbnailHeight:350,
		thumbnailXOffset3D:80,
		thumbnailXSpace3D:128,
		thumbnailZOffset3D:225,
		thumbnailZSpace3D:154,
