...
(function (window){
	...
	var FWDR3DCovComboBox = function(parent, props_obj){
		
		var self = this;
		var prototype = FWDR3DCovComboBox.prototype;
		...
		this.setupMainContainers = function(){
			...
			self.mainHolder_do = new FWDR3DCovDisplayObject("div");
			self.mainHolder_do.setOverflow("visible");
			self.addChild(self.mainHolder_do);
			
			self.mainButtonsHolder_do = new FWDR3DCovDisplayObject("div");
			...
			self.selector_do = new FWDR3DCovComboBoxSelector(
					self.arrowW,
					self.arrowH,
					self.arrowN_str,
					self.arrowS_str,
			...
			self.selector_do.addListener(FWDR3DCovComboBoxSelector.MOUSE_DOWN, self.openMenuHandler);
			
			for(var i=0; i<self.totalButtons; i++){
				FWDR3DCovComboBoxButton.setPrototype();
				button_do = new FWDR3DCovComboBoxButton(
				...
				button_do.addListener(FWDR3DCovComboBoxButton.MOUSE_DOWN, self.buttonOnMouseDownHandler);
		...
		this.buttonOnMouseDownHandler = function(e){
			...
			self.dispatchEvent(FWDR3DCovComboBox.BUTTON_PRESSED, {id:self.curId});
		};
		...
		this.openMenuHandler = function(){
			...
			self.dispatchEvent(FWDR3DCovComboBox.OPEN);
		};
		...
		this.checkOpenedMenu = function(e){
			...
			if(!FWDR3DCovUtils.hitTest(self.screen, viewportMouseCoordinates.screenX, viewportMouseCoordinates.screenY)){
		...
		this.showFirstTime = function(){
			...
			FWDR3DCovModTweenMax.to(self.mainHolder_do, .8, {y:0, ease:Expo.easeInOut});	
		};
		...
		this.hide = function(animate, overwrite){
			...
			if(animate){
				FWDR3DCovModTweenMax.to(self.buttonsHolder_do, .6, {y:- self.totalButtonsHeight, ease:Expo.easeInOut});	
				FWDR3DCovModTweenMax.to(self.mainButtonsHolder_do, .6, {h:0, ease:Expo.easeInOut});	
				FWDR3DCovModTweenMax.to(self, .6, {h:self.buttonHeight, ease:Expo.easeInOut});	
			}else{
		...
		this.show = function(animate, overwrite){
			...
			if(animate){
				FWDR3DCovModTweenMax.to(self.buttonsHolder_do, .6, {y:0, ease:Expo.easeInOut});
				FWDR3DCovModTweenMax.to(self.mainButtonsHolder_do, .6, {h:self.totalButtonsHeight + self.buttonHeight, ease:Expo.easeInOut});	
				FWDR3DCovModTweenMax.to(self, .6, {h:self.totalButtonsHeight + self.buttonHeight, ease:Expo.easeInOut});	
			}else{
		...
		this.destroy = function(){
			...
			FWDR3DCovModTweenMax.killTweensOf(self.mainHolder_do);
			FWDR3DCovModTweenMax.killTweensOf(self.buttonsHolder_do);	
			FWDR3DCovModTweenMax.killTweensOf(self.mainButtonsHolder_do);
			...
			self = null;
			prototype = null;
			FWDR3DCovComboBox.prototype = null;
		};
	};
	
	/* set prototype */
	FWDR3DCovComboBox.setPrototype =  function(){
		FWDR3DCovComboBox.prototype = new FWDR3DCovDisplayObject3D("div");
	};

	FWDR3DCovComboBox.OPEN = "open";
	FWDR3DCovComboBox.HIDE_COMPLETE = "infoWindowHideComplete";
	FWDR3DCovComboBox.BUTTON_PRESSED = "buttonPressed";

	FWDR3DCovComboBox.prototype = null;
	window.FWDR3DCovComboBox = FWDR3DCovComboBox;
	
}(window));/* FWDR3DCovComboBoxButton */
(function (){
var FWDR3DCovComboBoxButton = function(
			label1, 
		...
		var self = this;
		var prototype = FWDR3DCovComboBoxButton.prototype;
		...
		self.setupMainContainers = function(){
			...
			self.bk_sdo = new FWDR3DCovSimpleDisplayObject("div");
		...
		self.onMouseOver = function(e){
			...
			if(!e.pointerType || e.pointerType == e.MSPOINTER_TYPE_MOUSE){
				FWDR3DCovModTweenMax.killTweensOf(self.text_sdo);
				self.setSelectedState(true);
				self.dispatchEvent(FWDR3DCovComboBoxButton.MOUSE_OVER);
			}
		};
		...
		self.onMouseOut = function(e){
			...
			if(!e.pointerType || e.pointerType == e.MSPOINTER_TYPE_MOUSE){
				...
				self.dispatchEvent(FWDR3DCovComboBoxButton.MOUSE_OUT);
			}
		};
		...
		self.onClick = function(e){
			if(self.isDisabled_bl) return;
			if(e.preventDefault) e.preventDefault();
			self.dispatchEvent(FWDR3DCovComboBoxButton.CLICK);
		};
		
		self.onMouseDown = function(e){
			if(self.isDisabled_bl) return;
			if(e.preventDefault) e.preventDefault();
			self.dispatchEvent(FWDR3DCovComboBoxButton.MOUSE_DOWN, {e:e});
		};
		...
		this.setSelectedState = function(animate){
			if(animate){
				FWDR3DCovModTweenMax.to(self.colorObj, .6, {colorProps:{color1:self.backgroundSelectedColor_str1, color2:self.backgroundSelectedColor_str2}, ease:Quart.easeOut, onUpdate:self.applyProps});
				FWDR3DCovModTweenMax.to(self.text_sdo.screen, .6, {css:{color:self.textSelectedColor_str}, ease:Quart.easeOut});
			}else{
		...
		this.setNormalState = function(animate){
			if(animate){
				FWDR3DCovModTweenMax.to(self.colorObj, .6, {colorProps:{color1:self.backgroundNormalColor_str1, color2:self.backgroundNormalColor_str2}, ease:Quart.easeOut, onUpdate:self.applyProps});
				FWDR3DCovModTweenMax.to(self.text_sdo.screen, .6, {css:{color:self.textNormalColor_str}, ease:Quart.easeOut});
			}else{
		...
		self.destroy = function(){
			...
			FWDR3DCovModTweenMax.killTweensOf(self.text_sdo.screen);
			FWDR3DCovModTweenMax.killTweensOf(self.bk_sdo.screen);
			...
			self = null;
			prototype = null;
			FWDR3DCovComboBoxButton.prototype = null;
		};
	...
	/* set prototype */
	FWDR3DCovComboBoxButton.setPrototype = function(){
		FWDR3DCovComboBoxButton.prototype = new FWDR3DCovDisplayObject("div");
	};
	
	FWDR3DCovComboBoxButton.FIRST_BUTTON_CLICK = "onFirstClick";
	FWDR3DCovComboBoxButton.SECOND_BUTTON_CLICK = "secondButtonOnClick";
	FWDR3DCovComboBoxButton.MOUSE_OVER = "onMouseOver";
	FWDR3DCovComboBoxButton.MOUSE_OUT = "onMouseOut";
	FWDR3DCovComboBoxButton.MOUSE_DOWN = "onMouseDown";
	FWDR3DCovComboBoxButton.CLICK = "onClick";
	
	FWDR3DCovComboBoxButton.prototype = null;
	window.FWDR3DCovComboBoxButton = FWDR3DCovComboBoxButton;
}(window));/* FWDR3DCovComboBoxSelector */
(function (){
var FWDR3DCovComboBoxSelector = function(
			arrowW,
			arrowH,
			arrowN_str,
			arrowS_str,
		...
		var self = this;
		var prototype = FWDR3DCovComboBoxSelector.prototype;
		...
		self.setupMainContainers = function(){
			...
			self.arrowN_sdo = new FWDR3DCovSimpleDisplayObject("div");
			self.arrowN_sdo.screen.style.backgroundImage = "url(" + self.arrowN_str + ")";
			self.arrowS_sdo = new FWDR3DCovSimpleDisplayObject("div");
		...
		self.onMouseOver = function(e){
			...
			if(!e.pointerType || e.pointerType == e.MSPOINTER_TYPE_MOUSE){
				FWDR3DCovModTweenMax.killTweensOf(self.text_sdo);
				self.setSelectedState(true);
				self.dispatchEvent(FWDR3DCovComboBoxSelector.MOUSE_OVER);
			}
		};
		...
		self.onMouseOut = function(e){
			...
			if(!e.pointerType || e.pointerType == e.MSPOINTER_TYPE_MOUSE){
				FWDR3DCovModTweenMax.killTweensOf(self.text_sdo);
				self.setNormalState(true);
				self.dispatchEvent(FWDR3DCovComboBoxSelector.MOUSE_OUT);
			}
		};
		...
		self.onClick = function(e){
			...
			if(self.isDisabled_bl) return;
			if(e.preventDefault) e.preventDefault();
			self.dispatchEvent(FWDR3DCovComboBoxSelector.CLICK);
		};
		
		self.onMouseDown = function(e){
			if(self.isDisabled_bl) return;
			if(e.preventDefault) e.preventDefault();
			self.dispatchEvent(FWDR3DCovComboBoxSelector.MOUSE_DOWN, {e:e});
		};
		...
		this.setSelectedState = function(animate){
			if(animate){
				FWDR3DCovModTweenMax.to(self.colorObj, .6, {colorProps:{color1:self.backgroundSelectedColor_str1, color2:self.backgroundSelectedColor_str2}, ease:Quart.easeOut, onUpdate:self.applyProps});
				FWDR3DCovModTweenMax.to(self.text_sdo.screen, .6, {css:{color:self.textSelectedColor_str}, ease:Quart.easeOut});
				FWDR3DCovModTweenMax.to(self.arrowS_sdo, .6, {alpha:1, ease:Quart.easeOut});
			}else{
				self.bk_sdo.setCSSGradient(self.backgroundSelectedColor_str, self.backgroundNormalColor_str);
				self.text_sdo.getStyle().color = self.textSelectedColor_str;
		...
		this.setNormalState = function(animate){
			if(animate){
				FWDR3DCovModTweenMax.to(self.colorObj, .6, {colorProps:{color1:self.backgroundNormalColor_str1, color2:self.backgroundNormalColor_str2}, ease:Quart.easeOut, onUpdate:self.applyProps});
				FWDR3DCovModTweenMax.to(self.text_sdo.screen, .6, {css:{color:self.textNormalColor_str}, ease:Quart.easeOut});
				FWDR3DCovModTweenMax.to(self.arrowS_sdo, .6, {alpha:0, ease:Quart.easeOut});
			}else{
				self.bk_sdo.setCSSGradient(self.backgroundNormalColor_str, self.backgroundSelectedColor_str);
				self.text_sdo.getStyle().color = self.textNormalColor_str;
		...
		this.disable = function(){
			...
			if(FWDR3DCovUtils.hasTransform2d){
				FWDR3DCovModTweenMax.to(self.arrowN_sdo.screen, .6, {css:{rotation:180}, ease:Quart.easeOut});
				FWDR3DCovModTweenMax.to(self.arrowS_sdo.screen, .6, {css:{rotation:180}, ease:Quart.easeOut});
			}
			self.setButtonMode(false);
		};
		...
		this.enable = function(){
			...
			self.setNormalState(true);
			if(FWDR3DCovUtils.hasTransform2d){
				FWDR3DCovModTweenMax.to(self.arrowN_sdo.screen, .6, {css:{rotation:0}, ease:Quart.easeOut});
				FWDR3DCovModTweenMax.to(self.arrowS_sdo.screen, .6, {css:{rotation:0}, ease:Quart.easeOut});
			}
			self.setButtonMode(true);
		};
		...
		self.destroy = function(){
			...
			FWDR3DCovModTweenMax.killTweensOf(self.text_sdo);
			FWDR3DCovModTweenMax.killTweensOf(self.colorObj);
			self.text_sdo.destroy();
			...
			self = null;
			prototype = null;
			FWDR3DCovComboBoxSelector.prototype = null;
		};
	...
	/* set prototype */
	FWDR3DCovComboBoxSelector.setPrototype = function(){
		FWDR3DCovComboBoxSelector.prototype = new FWDR3DCovDisplayObject("div");
	};
	
	FWDR3DCovComboBoxSelector.FIRST_BUTTON_CLICK = "onFirstClick";
	FWDR3DCovComboBoxSelector.SECOND_BUTTON_CLICK = "secondButtonOnClick";
	FWDR3DCovComboBoxSelector.MOUSE_OVER = "onMouseOver";
	FWDR3DCovComboBoxSelector.MOUSE_OUT = "onMouseOut";
	FWDR3DCovComboBoxSelector.MOUSE_DOWN = "onMouseDown";
	FWDR3DCovComboBoxSelector.CLICK = "onClick";
	
	FWDR3DCovComboBoxSelector.prototype = null;
	window.FWDR3DCovComboBoxSelector = FWDR3DCovComboBoxSelector;
}(window));/* FWDR3DCovComplexButton */
(function (window){
var FWDR3DCovComplexButton = function(
			n1Img, 
			s1Img, 
			n2Img, 
			s2Img, 
		...
		var self = this;
		var prototype = FWDR3DCovComplexButton.prototype;
		...
		this.onMouseOver = function(e){
			if(!e.pointerType || e.pointerType == e.MSPOINTER_TYPE_MOUSE){
				FWDR3DCovModTweenMax.killTweensOf(self.n1_do);
				FWDR3DCovModTweenMax.killTweensOf(self.n2_do);
				FWDR3DCovModTweenMax.to(self.n1_do, .8, {alpha:0, ease:Expo.easeOut});
				FWDR3DCovModTweenMax.to(self.n2_do, .8, {alpha:0, ease:Expo.easeOut});
			}
		};
		...
		this.onMouseOut = function(e){
			if(!e.pointerType || e.pointerType == e.MSPOINTER_TYPE_MOUSE){
				...
				FWDR3DCovModTweenMax.to(self.n1_do, .8, {alpha:1, delay:dl, ease:Expo.easeOut});	
				FWDR3DCovModTweenMax.to(self.n2_do, .8, {alpha:1, delay:dl, ease:Expo.easeOut});	
			}
		};
		
		this.onMouseDown = function(e){
			if(self.disptachMainEvent_bl){
				self.dispatchEvent(FWDR3DCovComplexButton.CLICK);
			}else{
		...
		this.toggleButton = function(){
			if(this.currentState == 1){
				...
				this.currentState = 0;
				this.dispatchEvent(FWDR3DCovComplexButton.SECOND_BUTTON_CLICK);
			}else{
				this.firstButton_do.setX(3000);
				this.secondButton_do.setX(0);
				this.currentState = 1;
				this.dispatchEvent(FWDR3DCovComplexButton.FIRST_BUTTON_CLICK);
			}
		};
		...
		this.destroy = function(){
			...
			self = null;
			prototype = null;
			FWDR3DCovComplexButton.prototype = null;
		};
	...
	/* set prototype */
	FWDR3DCovComplexButton.setPrototype = function(){
		FWDR3DCovComplexButton.prototype = new FWDR3DCovDisplayObject("div");
	};
	
	FWDR3DCovComplexButton.FIRST_BUTTON_CLICK = "onFirstClick";
	FWDR3DCovComplexButton.SECOND_BUTTON_CLICK = "secondButtonOnClick";
	FWDR3DCovComplexButton.CLICK = "onClick";
	
	FWDR3DCovComplexButton.prototype = null;
	window.FWDR3DCovComplexButton = FWDR3DCovComplexButton;
}(window));/* Context menu */
(function (){
	var FWDR3DCovContextMenu = function(e, showMenu){
		...
		this.contextMenuWindowOnMouseDownHandler = function(e){
			...
			if(!FWDR3DCovUtils.hitTest(self.menu_do.screen, screenX, screenY)){
				if(window.removeEventListener){
		...
		this.mouseOverHandler = function(){
			...
			FWDR3DCovModTweenMax.to(self.normalMenu_do, .8, {alpha:0, ease:Expo.easeOut});
			FWDR3DCovModTweenMax.to(self.selectedMenu_do, .8, {alpha:1, ease:Expo.easeOut});
		};
		
		this.mouseOutHandler = function(){
			FWDR3DCovModTweenMax.to(self.normalMenu_do, .8, {alpha:1, ease:Expo.easeOut});
			FWDR3DCovModTweenMax.to(self.selectedMenu_do, .8, {alpha:0, ease:Expo.easeOut});
		};
		...
		this.destroy = function(){
			...
			if(this.menu_do){
				FWDR3DCovModTweenMax.killTweensOf(self.normalMenu_do);
				FWDR3DCovModTweenMax.killTweensOf(self.selectedMenu_do);
...
(function(window)
{
	var FWDR3DCovData = function(props)
	{
		var self = this;
		...
		this.parseProperties = function()
		{
			...
			{
				errorMessage = "Coverflow data list id is not defined in FWDR3DCov3DCoverflow constructor function!";
				self.dispatchEvent(FWDR3DCovData.LOAD_ERROR, {text : errorMessage});
				
				return;
			};

			// set the root element of the coverflow list.
			self.rootElement = FWDR3DCovUtils.getChildById(self.propsObj.coverflowDataListDivId);
			...
			self.swipeSpeed = self.propsObj.coverflowMobileSwipeSpeed || 1;
			...
			{
				self.comboBoxPosition = FWDR3DCovUtils.trim(self.propsObj.comboBoxPosition).toLowerCase();
			}
			else
			{
			...
			// parse datalist.
			var dataListAr = FWDR3DCovUtils.getChildrenFromAttribute(self.rootElement, "data-cat");
			
			if (!dataListAr)
			{
				errorMessage = "At least one ul tag with the attribute <font color='#FFFFFF'>data-cat</font> must be defined in the playlist.";
				self.dispatchEvent(FWDR3DCovData.LOAD_ERROR, {text:errorMessage});
				return;
			}
			...
			for (var i=0; i<totalDataLists; i++)
			{
				...
				for (var j=0; j<totalChildren; j++)
				{
					...
					if (self.showThumbnailsHtmlContent)
					{
						...
						for (var k=0; k<totalInnerChildren; k++)
						{
							...
							if(FWDR3DCovUtils.hasAttribute(childKidsAr[k], "data-html-content"))
							{
								hasError = false;
					...
					else
					{
						...
						for (var k=0; k<totalInnerChildren; k++)
						{
							...
							if (FWDR3DCovUtils.hasAttribute(childKidsAr[k], "data-thumbnail-path"))
							{
								hasError = false;
								obj.thumbPath = FWDR3DCovUtils.trim(FWDR3DCovUtils.getAttributeValue(childKidsAr[k], "data-thumbnail-path"));
					...
					if (self.showText)
					{
						...
						for (var k=0; k<totalInnerChildren; k++)
						{
							...
							if (FWDR3DCovUtils.hasAttribute(childKidsAr[k], "data-thumbnail-text"))
							{
								hasError = false;
						...
						obj.textTitleOffset = parseInt(FWDR3DCovUtils.trim(FWDR3DCovUtils.getAttributeValue(mediaKid, "data-thumbnail-text-title-offset")));
						obj.textDescriptionOffsetTop = parseInt(FWDR3DCovUtils.trim(FWDR3DCovUtils.getAttributeValue(mediaKid, "data-thumbnail-text-offset-top")));
						obj.textDescriptionOffsetBottom = parseInt(FWDR3DCovUtils.trim(FWDR3DCovUtils.getAttributeValue(mediaKid, "data-thumbnail-text-offset-bottom")));
						
						if (FWDR3DCovUtils.trim(obj.thumbText) == "")
						{
							obj.emptyText = true;
						}
						else
					...
					for (var k=0; k<totalInnerChildren; k++)
					{
						...
						if (FWDR3DCovUtils.hasAttribute(childKidsAr[k], "data-type"))
						{
							hasError = false;
							obj.mediaType = FWDR3DCovUtils.trim(FWDR3DCovUtils.getAttributeValue(childKidsAr[k], "data-type"));
					...
					if (obj.mediaType != "none")
					{
						...
						for (var k=0; k<totalInnerChildren; k++)
						{
							...
							if (FWDR3DCovUtils.hasAttribute(childKidsAr[k], "data-url"))
							{
								hasError = false;
								mediaKid = childKidsAr[k];
					...
					secondObj.dataType = FWDR3DCovUtils.trim(FWDR3DCovUtils.getAttributeValue(mediaKid, "data-type"));
					secondObj.url = FWDR3DCovUtils.trim(FWDR3DCovUtils.getAttributeValue(mediaKid, "data-url"));
					secondObj.target = FWDR3DCovUtils.getAttributeValue(mediaKid, "data-target");	
					secondObj.info = FWDR3DCovUtils.getAttributeValue(mediaKid, "data-info");
					...
					for (var k=0; k<totalInnerChildren; k++)
					{
						if(FWDR3DCovUtils.hasAttribute(childKidsAr[k], "data-info"))
						{
				...
				self.categoriesAr[i] = FWDR3DCovUtils.getAttributeValue(curUlElem, "data-cat") || "not defined!";
			...
			{
				self.dispatchEvent(FWDR3DCovData.LOAD_ERROR, {text:"Coverflow graphics skin path is not defined in FWDR3DCov3DCoverflow constructor function!"});
				return;
			}
		...
		this.onImageLoadHandler = function(e)
		{
			...
			else if (self.countLoadedGraphics == 4)
			{
				...
				self.dispatchEvent(FWDR3DCovData.PRELOADER_LOAD_DONE);
			}
			...
			{
				self.dispatchEvent(FWDR3DCovData.LOAD_DONE);
			}
		};
		...
		this.onImageLoadErrorHandler = function(e)
		{
			...
			self.dispatchEvent(FWDR3DCovData.LOAD_ERROR, err);
		};
		...
		this.checkForAttribute = function(e, attr)
		{
			...
			if (!test)
			{
				self.dispatchEvent(FWDR3DCovData.LOAD_ERROR, {text:"Element  with attribute <font color='#FFFFFF'>" + attr + "</font> is not defined."});
				return;
			}
...
(function (window){
	 ...
	 */
	var FWDR3DCovDisplayObject = function(type, position, overflow, display){
		...
		if(FWDR3DCovUtils.isFirefox) self.hasTransform3d_bl = false;
		if(FWDR3DCovUtils.isFirefox) self.hasTransform2d_bl = false;
		...
		this.setY = function(val){
			...
			if(self.isMobile_bl){
				self.screen.style[self.transform] = 'translate(' + self.x + 'px,' + self.y + 'px)';
			}else if(self.hasTransform3d_bl && !FWDR3DCovUtils.isAndroid){
				self.screen.style[self.transform] = 'translate3d(' + self.x + 'px,' + self.y + 'px,0)';
			}else if(self.hasTransform2d_bl){
				self.screen.style[self.transform] = 'translate(' + self.x + 'px,' + self.y + 'px)';
			}else{
		...
		this.addChildAtZero = function(e){
			if(this.numChildren == 0){
				...
				if(this.contains(e)){this.children_ar.splice(FWDR3DCovUtils.indexOfArray(this.children_ar, e), 1);}	
	...
	window.FWDR3DCovDisplayObject = FWDR3DCovDisplayObject;
}(window));/* Display object */
(function (window){
	/*
	 * @ type values: div, img.
	 ...
	 * @ display values: block, inline-block, this applies only if the position is relative.
	 */
	var FWDR3DCovDisplayObject3D = function(type, position, overflow, display){
	...
	window.FWDR3DCovDisplayObject3D = FWDR3DCovDisplayObject3D;
}(window));(function (){
...
(function (window){
	...
	var FWDR3DCovInfoWindow = function(
			margins,
			backgroundColor_str,
			backgroundOpacity,
			borderRadius,
		...
		var self = this;
		var prototype = FWDR3DCovInfoWindow.prototype;
		...
		this.resize = function(maxWidth, maxHeight, animate){
			...
			FWDR3DCovModTweenMax.killTweensOf(self.obj);	
			if(animate){
				FWDR3DCovModTweenMax.to(self.obj, .8, {delay:.1, currentWidth:self.maxWidth, onUpdate:self.onTweenUpdate, ease:Expo.easeInOut});
			}else{
		...
		this.destroy = function(){
			...
			self = null;
			prototype = null;
			FWDR3DCovInfoWindow.prototype = null;
		};
	};
	
	/* set prototype */
	FWDR3DCovInfoWindow.setPrototype =  function(){
		FWDR3DCovInfoWindow.prototype = new FWDR3DCovDisplayObject("div");
	};


	FWDR3DCovInfoWindow.HIDE_COMPLETE = "infoWindowHideComplete";

	FWDR3DCovInfoWindow.prototype = null;
	window.FWDR3DCovInfoWindow = FWDR3DCovInfoWindow;
	
}(window));(function (window){
	
	var FWDR3DCovLightBox = function(props){
		
		var self = this;
		var prototype = FWDR3DCovLightBox.prototype;
		...
		this.resizeHandler = function(){
		    ...
		    var viewPortSize = FWDR3DCovUtils.getViewportSize();
			var scrollOffsets = FWDR3DCovUtils.getScrollOffsets();
		...
		this.show = function(id){
			...
			this.dispatchEvent(FWDR3DCovLightBox.SHOW_START);
		};
		...
		this.showCurrentItem = function(){	 
			...
			if(this.data_ar[this.id].dataType.toLowerCase() == "iframe"){
				...
				this.type_str = FWDR3DCovLightBox.IFRAME;
				...
				this.type_str = FWDR3DCovLightBox.IMAGE;
				...
				this.type_str = FWDR3DCovLightBox.YOUTUBE;
				if(self.forceRoundBorderToIframe_bl && self.borderRadius != 0){
					self.itemsBorder_do.getStyle().borderRadius = self.borderRadius + "px";
				}else if(self.borderRadius != 0){
					 self.itemsBorder_do.getStyle().borderRadius = "0px";
				...
				this.type_str = FWDR3DCovLightBox.VIMEO;
				if(self.forceRoundBorderToIframe_bl && self.borderRadius != 0){
					self.itemsBorder_do.getStyle().borderRadius = self.borderRadius + "px";
				}else if(self.borderRadius != 0){
					 self.itemsBorder_do.getStyle().borderRadius = "0px";
		...
		this.createItem = function(){		
			...
			if(this.type_str == FWDR3DCovLightBox.IMAGE){
				if(this.prevItem_do){
				...
				this.loadImage();
			}else if(this.type_str == FWDR3DCovLightBox.YOUTUBE || this.type_str == FWDR3DCovLightBox.VIMEO || this.type_str == FWDR3DCovLightBox.IFRAME){
				...
				if(this.firstTimeShowed_bl){
					...
					this.animMainDos();
					this.showVideoId_to = setTimeout(this.showIframeContent, 900);
					if(this.showZoomButton_bl && (this.type_str == FWDR3DCovLightBox.YOUTUBE || this.type_str == FWDR3DCovLightBox.VIMEO || self.type_str == FWDR3DCovLightBox.IFRAME)){
						var index = FWDR3DCovUtils.indexOfArray(this.buttons_ar, this.zoomButton_do);
						if(index != -1){
				...
				if(self.infoWindow_do){
					if(this.mainItemsHolder_do.contains(self.infoWindow_do) && this.infoWindow_do.isShowed_bl){
						this.infoWindow_do.setText(this.data_ar[self.id].infoText, this.finalWidth, this.finalHeight, false, this.type_str != FWDR3DCovLightBox.IMAGE);
					};
				}
			}
		...
		this.createIframeHolder = function(){
			...
			if(this.type_str == FWDR3DCovLightBox.IFRAME && this.isMobile_bl){
		...
		this.imageLoadComplete = function(e){
			...
			if(self.prevItem_do){
				if(!self.isMobile_bl && self.prevItem_do.type == "img") FWDR3DCovModTweenMax.to(self.prevItem_do, .6, {alpha:0});
			}
			
			self.originalWidth = self.image_img.width;
			self.originalHeight = self.image_img.height;
			
			self.currentItem_do = new FWDR3DCovDisplayObject("img");
			...
			if(self.firstTimeShowed_bl){
				...
				FWDR3DCovModTweenMax.to(self.currentItem_do, .6, {alpha:1, delay:.8});
			...
			if(self.infoWindow_do && self.infoWindow_do.isShowed_bl){
				self.infoWindow_do.setText(self.data_ar[self.id].infoText, self.finalWidth, self.finalHeight, true, self.type_str != FWDR3DCovLightBox.IMAGE);
			};
		...
		this.animMainDos = function(){
			FWDR3DCovModTweenMax.to(this.mainItemsHolder_do, .8, {delay:.1,x:self.finalX, y:self.finalY, w:self.finalWidth, h:self.finalHeight, ease:Expo.easeInOut});
			FWDR3DCovModTweenMax.to(this.itemsBackground_do, .8, {delay:.1, x:self.borderSize, y:self.borderSize, w:self.finalWidth - (self.borderSize * 2), h:self.finalHeight - (self.borderSize * 2), ease:Expo.easeInOut});
			FWDR3DCovModTweenMax.to(this.itemsBorder_do, .8, {delay:.1, w:self.finalWidth, h:self.finalHeight, ease:Expo.easeInOut});
			FWDR3DCovModTweenMax.to(this.itemsHolder_do, .8, {delay:.1, x:self.borderSize, y:self.borderSize, w:self.finalWidth - (self.borderSize * 2), h:self.finalHeight - (self.borderSize * 2), ease:Expo.easeInOut});
			if(!this.isMobile_bl && this.prevItem_do.type == "img") FWDR3DCovModTweenMax.to(self.prevItem_do, .8, {delay:.1, x: (self.finalWidth -  (self.borderSize * 2) - self.prevItem_do.getWidth())/2, y:(self.finalHeight -  (self.borderSize * 2) - self.prevItem_do.getHeight())/2, ease:Expo.easeInOut});
		};
		...
		this.showIframeContent = function(){
			...
			if(self.type_str == FWDR3DCovLightBox.YOUTUBE){
			...
			}else if(self.type_str == FWDR3DCovLightBox.VIMEO){
				if(self.videoAutoPlay_bl){
			...
			}else if(self.type_str == FWDR3DCovLightBox.IFRAME){
		...
		this.cleanChildren = function(index){
			...
			while(self.itemsHolder_do.getNumChildren() > index){
				...
				FWDR3DCovModTweenMax.killTweensOf(child);
		...
		this.maximizeOrMinimize = function(){
			...
			if(this.isMaximized_bl){
				...
				this.dispatchEvent(FWDR3DCovLightBox.MINIMIZE_START);
			}else{
				...
				if(this.isMobile_bl){
					...
					FWDR3DCovModTweenMax.to(this.currentItem_do, .8, { x:finalX, y:finalY, w:finalWidth, h:finalHeight, ease:Expo.easeInOut});
				}else{
					this.zoomButton_do.isMaximized_bl = true;
					if(scaleX >= scaleY){
						FWDR3DCovModTweenMax.to(this.currentItem_do, .8, {x:finalX, w:finalWidth, h:finalHeight, ease:Expo.easeInOut});
					}else if(scaleX < scaleY){
						FWDR3DCovModTweenMax.to(this.currentItem_do, .8, {y:finalY, w:finalWidth, h:finalHeight, ease:Expo.easeInOut});
					}
		...
		this.maximizeCompleteHandler = function(){
			...
			self.dispatchEvent(FWDR3DCovLightBox.MAXIMIZE_COMPLETE);
		...
		this.updateMaximizeImageOnMouseMovedHandler = function(e){
			...
			FWDR3DCovModTweenMax.to(self.zoomButton_do, .2, {x:self.globalXMousePosition - parseInt(self.buttonWidth/2), y:self.globalYMousePosition - parseInt(self.buttonHeight/2)});
		};
		...
		this.updateMaximizedImageHandler = function(){
			...
			if(scaleX <= scaleY){
				...
				FWDR3DCovModTweenMax.to(self.currentItem_do, .4, {x:targetX});
			}else {
				targetY = Math.round(((self.stageHeight - self.currentItem_do.getHeight()) * self.percentY));
				if(isNaN(targetY)) return;
				FWDR3DCovModTweenMax.to(self.currentItem_do, .4, {y:targetY});
			}
		};
		...
		this.onShowInfoButtonPressedHandler = function(e){
			self.infoWindow_do.setText(self.data_ar[self.id].infoText, self.finalWidth, self.finalHeight, false, self.type_str != FWDR3DCovLightBox.IMAGE);
		...
		this.positionButtons = function(animate){
			...
			for(var i=0; i<totalButtons; i++){
				...
				FWDR3DCovModTweenMax.killTweensOf(button);
				...
				if(button){
					if(animate){
						FWDR3DCovModTweenMax.to(button, .8, {x:button.finalX, y:button.finalY, delay:.1, ease:Expo.easeInOut});
					}else{
		...
		this.hide = function(){
			...
			if(this.type_str == FWDR3DCovLightBox.YOUTUBE || this.type_str == FWDR3DCovLightBox.VIMEO || self.type_str == FWDR3DCovLightBox.IFRAME){
			...
			}else if(this.type_str == FWDR3DCovLightBox.IMAGE){
			...
			this.isTweening_bl = true;
			this.firstTimeShowed_bl = true;
			self.dispatchEvent(FWDR3DCovLightBox.HIDE_START);
		};
		
		this.hideButtons = function(animate){
			if(animate){
				FWDR3DCovModTweenMax.to(this.closeButton_do, .8, {x:this.stageWidth, ease:Expo.easeInOut});
				if(this.infoButton_do) FWDR3DCovModTweenMax.to(this.infoButton_do, .8, {x:this.stageWidth, ease:Expo.easeInOut});
				if(this.slideshowButtton_do) FWDR3DCovModTweenMax.to(this.slideshowButtton_do, .8, {x:this.stageWidth, ease:Expo.easeInOut});
				if(this.zoomButton_do) FWDR3DCovModTweenMax.to(this.zoomButton_do, .8, {x:this.stageWidth, ease:Expo.easeInOut});
				...
				if(this.slideShowPreloader_do) FWDR3DCovModTweenMax.to(this.slideShowPreloader_do, .8, {x:this.stageWidth, ease:Expo.easeInOut});
			}else{
		...
		this.showButtons = function(){
			...
			if(this.zoomButton_do && (this.type_str != FWDR3DCovLightBox.YOUTUBE || this.type_str != FWDR3DCovLightBox.VIMEO || self.type_str == FWDR3DCovLightBox.IFRAME)){
		...
		this.onHideComplete = function(){
			...
			self.dispatchEvent(FWDR3DCovLightBox.HIDE_COMPLETE);
		};
		...
		this.destroy = function(){
			...
			self = null;
			prototype = null;
			FWDR3DCovLightBox.prototype = null;
		};
	...
	/* set prototype */
	FWDR3DCovLightBox.setPrototype =  function(){
		FWDR3DCovLightBox.prototype = new FWDR3DCovDisplayObject("div");
	};

	FWDR3DCovLightBox.YOUTUBE = "youtube";
	FWDR3DCovLightBox.VIMEO = "vimeo";
	FWDR3DCovLightBox.IMAGE = "image_img";
	FWDR3DCovLightBox.IFRAME = "htmlIframe";
	FWDR3DCovLightBox.MAXIMIZE_COMPLETE = "maximizeComplete";
	FWDR3DCovLightBox.MINIMIZE_START = "minimizeStart";
	FWDR3DCovLightBox.SHOW_START = "showStart";
	FWDR3DCovLightBox.HIDE_COMPLETE = "hideComplete";
	FWDR3DCovLightBox.HIDE_START = "hideStart";
	
	
	FWDR3DCovLightBox.prototype = null;
	window.FWDR3DCovLightBox = FWDR3DCovLightBox;
 ...
 * UPDATES AND DOCS AT: http://www.greensock.com
 * 
 * Includes all of the following: TweenLite, FWDR3DCovModTweenMax, TimelineLite, TimelineMax, EasePack, CSSPlugin, RoundPropsPlugin, BezierPlugin, AttrPlugin, DirectionalRotationPlugin
 *
 * @license Copyright (c) 2008-2013, GreenSock. All rights reserved.
 * This work is subject to the terms at http://www.greensock.com/terms_of_use.html or for
 * Club GreenSock members, the software agreement that was issued with your membership.
...
(window._gsQueue || (window._gsQueue = [])).push( function() {
	...
	window._gsDefine("FWDR3DCovModTweenMax", ["core.Animation","core.SimpleTimeline","TweenLite"], function(Animation, SimpleTimeline, TweenLite) {
		
		var _slice = [].slice,
			FWDR3DCovModTweenMax = function(target, duration, vars) {
			...
			p = FWDR3DCovModTweenMax.prototype = TweenLite.to({}, 0.1, {}),
			_blankArray = [];

		FWDR3DCovModTweenMax.version = "1.9.7";
		p.constructor = FWDR3DCovModTweenMax;
		p.kill()._gc = false;
		FWDR3DCovModTweenMax.killTweensOf = FWDR3DCovModTweenMax.killDelayedCallsTo = TweenLite.killTweensOf;
		FWDR3DCovModTweenMax.getTweensOf = TweenLite.getTweensOf;
		FWDR3DCovModTweenMax.ticker = TweenLite.ticker;
...
//---- STATIC FUNCTIONS -----------------------------------------------------------------------------------------------------------
		...
		FWDR3DCovModTweenMax.to = function(target, duration, vars) {
			return new FWDR3DCovModTweenMax(target, duration, vars);
		};
		
		FWDR3DCovModTweenMax.from = function(target, duration, vars) {
			vars.runBackwards = true;
			vars.immediateRender = (vars.immediateRender != false);
			return new FWDR3DCovModTweenMax(target, duration, vars);
		};
		
		FWDR3DCovModTweenMax.fromTo = function(target, duration, fromVars, toVars) {
			toVars.startAt = fromVars;
			toVars.immediateRender = (toVars.immediateRender != false && fromVars.immediateRender != false);
			return new FWDR3DCovModTweenMax(target, duration, toVars);
		};
		
		FWDR3DCovModTweenMax.staggerTo = FWDR3DCovModTweenMax.allTo = function(targets, duration, vars, stagger, onCompleteAll, onCompleteAllParams, onCompleteAllScope) {
			stagger = stagger || 0;
			...
			for (i = 0; i < l; i++) {
				...
				a[i] = new FWDR3DCovModTweenMax(targets[i], duration, copy);
				delay += stagger;
			}
			return a;
		};
		
		FWDR3DCovModTweenMax.staggerFrom = FWDR3DCovModTweenMax.allFrom = function(targets, duration, vars, stagger, onCompleteAll, onCompleteAllParams, onCompleteAllScope) {
			vars.runBackwards = true;
			vars.immediateRender = (vars.immediateRender != false);
			return FWDR3DCovModTweenMax.staggerTo(targets, duration, vars, stagger, onCompleteAll, onCompleteAllParams, onCompleteAllScope);
		};
		
		FWDR3DCovModTweenMax.staggerFromTo = FWDR3DCovModTweenMax.allFromTo = function(targets, duration, fromVars, toVars, stagger, onCompleteAll, onCompleteAllParams, onCompleteAllScope) {
			toVars.startAt = fromVars;
			toVars.immediateRender = (toVars.immediateRender != false && fromVars.immediateRender != false);
			return FWDR3DCovModTweenMax.staggerTo(targets, duration, toVars, stagger, onCompleteAll, onCompleteAllParams, onCompleteAllScope);
		};
				
		FWDR3DCovModTweenMax.delayedCall = function(delay, callback, params, scope, useFrames) {
			return new FWDR3DCovModTweenMax(callback, 0, {delay:delay, onComplete:callback, onCompleteParams:params, onCompleteScope:scope, onReverseComplete:callback, onReverseCompleteParams:params, onReverseCompleteScope:scope, immediateRender:false, useFrames:useFrames, overwrite:0});
		};
		
		FWDR3DCovModTweenMax.set = function(target, vars) {
			return new FWDR3DCovModTweenMax(target, 0, vars);
		};
		
		FWDR3DCovModTweenMax.isTweening = function(target) {
		...
		var _getChildrenOf = function(timeline, includeTimelines) {
			...
			getAllTweens = FWDR3DCovModTweenMax.getAllTweens = function(includeTimelines) {
				return _getChildrenOf(Animation._rootTimeline, includeTimelines).concat( _getChildrenOf(Animation._rootFramesTimeline, includeTimelines) );
			};
		
		FWDR3DCovModTweenMax.killAll = function(complete, tweens, delayedCalls, timelines) {
			if (tweens == null) {
				tweens = true;
			}
		...
		FWDR3DCovModTweenMax.killChildTweensOf = function(parent, complete) {
			if (parent == null) {
				return;
			}
			...
			if (parent instanceof Array) {
				i = parent.length;
				while (--i > -1) {
					FWDR3DCovModTweenMax.killChildTweensOf(parent[i], complete);
				}
				return;
			}
			a = [];
		...
		FWDR3DCovModTweenMax.pauseAll = function(tweens, delayedCalls, timelines) {
			_changePause(true, tweens, delayedCalls, timelines);
		};
		
		FWDR3DCovModTweenMax.resumeAll = function(tweens, delayedCalls, timelines) {
...
//---- GETTERS / SETTERS ----------------------------------------------------------------------------------------------------------
		...
		p.duration = function(value) {
			if (!arguments.length) {
				return this._duration; //don't set _dirty = false because there could be repeats that haven't been factored into the _totalDuration yet. Otherwise, if you create a repeated FWDR3DCovModTweenMax and then immediately check its duration(), it would cache the value and the totalDuration would not be correct, thus repeats wouldn't take effect.
			}
...
(function(window) {
		...
		var _globals = window.GreenSockGlobals || window,
			 ...
			 * </script>
			 * <script src="js/greensock/v1.7/FWDR3DCovModTweenMax.js"></script>
			 * <script>
			 *     window.GreenSockGlobals = null; //reset it back to null so that the next load of FWDR3DCovModTweenMax affects the window and we can reference things directly like TweenLite.to(...)
			 * </script>
			 * <script src="js/greensock/v1.6/FWDR3DCovModTweenMax.js"></script>
			 * <script>
...
//----Animation getters/setters --------------------------------------------------------
		...
		p.duration = function(value) {
			...
			this._uncache(true); //true in case it's a FWDR3DCovModTweenMax or TimelineMax that has a repeat - we'll need to refresh the totalDuration.
...
}(window));/* FWDR3DCovScrollbar */
(function(window)
{
	var FWDR3DCovScrollbar = function(data, totalItems, curItemId)
	{
		var self = this;
		...
		{
			self.scrollbarTrackDO = new FWDR3DCovDisplayObject("div");
			...
			self.scrollbarTrackLeftDO = new FWDR3DCovSimpleDisplayObject("img");
			self.scrollbarTrackLeftDO.setScreen(self.trackLeftImg);
			self.scrollbarTrackDO.addChild(self.scrollbarTrackLeftDO);
			
			self.scrollbarTrackCenterDO = new FWDR3DCovSimpleDisplayObject("div");
			...
			self.scrollbarTrackRightDO = new FWDR3DCovSimpleDisplayObject("img");
		...
		{
			self.scrollbarHandlerDO = new FWDR3DCovDisplayObject("div");
			...
			self.scrollbarHandlerCenterNDO = new FWDR3DCovSimpleDisplayObject("div");
			...
			self.scrollbarHandlerRightSDO = new FWDR3DCovSimpleDisplayObject("img");
			...
			self.scrollbarHandlerRightNDO = new FWDR3DCovSimpleDisplayObject("img");
			...
			self.scrollbarHandlerTextDO = new FWDR3DCovDisplayObject("div");
			...
			self.scrollbarHandlerOverDO = new FWDR3DCovDisplayObject("div");
		...
		this.onScrollMouseOver = function()
		{
			...
			FWDR3DCovModTweenMax.to(self.scrollbarHandlerLeftNDO, .8, {alpha:0, ease : Expo.easeOut});
			FWDR3DCovModTweenMax.to(self.scrollbarHandlerCenterNDO, .8, {alpha:0, ease : Expo.easeOut});
			FWDR3DCovModTweenMax.to(self.scrollbarHandlerRightNDO, .8, {alpha:0, ease : Expo.easeOut});
			FWDR3DCovModTweenMax.to(self.scrollbarHandlerTextDO.screen, .8, {css : {color : self.textColorSelected}, ease : Expo.easeOut});
		};
		...
		this.onScrollMouseOut = function()
		{
			...
			FWDR3DCovModTweenMax.to(self.scrollbarHandlerLeftNDO, .8, {alpha:1, ease : Expo.easeOut});
			FWDR3DCovModTweenMax.to(self.scrollbarHandlerCenterNDO, .8, {alpha:1, ease : Expo.easeOut});
			FWDR3DCovModTweenMax.to(self.scrollbarHandlerRightNDO, .8, {alpha:1, ease : Expo.easeOut});
			FWDR3DCovModTweenMax.to(self.scrollbarHandlerTextDO.screen, .8, {css : {color : self.textColorNormal}, ease : Expo.easeOut});
		};
		...
		this.onScrollMouseDown = function(e)
		{
			...
			FWDR3DCovModTweenMax.killTweensOf(self.scrollbarHandlerDO);
		...
		this.onScrollMove = function(e)
		{
			...
			{
				self.dispatchEvent(FWDR3DCovScrollbar.MOVE, {id:self.curItemId});
		...
		this.gotoItem = function(id, animate)
		{
			...
			if (self.prevCurItemId != self.curItemId)
			{
				...
				{
					FWDR3DCovModTweenMax.killTweensOf(self.scrollbarHandlerDO);
					FWDR3DCovModTweenMax.to(self.scrollbarHandlerDO, .8, {x : Math.floor(self.curItemId * (self.trackWidth - self.handlerWidth) / (self.totalItems - 1)), ease : Expo.easeOut});
				}
				else
				{
					self.scrollbarHandlerDO.setX(Math.floor(self.curItemId * (self.trackWidth - self.handlerWidth) / (self.totalItems - 1)));
		...
		this.destroy = function()
		{
			...
			FWDR3DCovModTweenMax.killTweensOf(self.scrollbarHandlerLeftNDO);
			FWDR3DCovModTweenMax.killTweensOf(self.scrollbarHandlerCenterNDO);
			FWDR3DCovModTweenMax.killTweensOf(self.scrollbarHandlerRightNDO);
			FWDR3DCovModTweenMax.killTweensOf(self.scrollbarHandlerDO);
			FWDR3DCovModTweenMax.killTweensOf(self.scrollbarHandlerTextDO.screen);
	...
	{
		FWDR3DCovScrollbar.prototype = new FWDR3DCovDisplayObject("div");
	};
...
(function (window){
	 ...
	 */
	var FWDR3DCovSimpleDisplayObject = function(type, position, overflow, display){
	...
	window.FWDR3DCovSimpleDisplayObject = FWDR3DCovSimpleDisplayObject;
}(window));/* FWDR3DCovSlideshowButton */
(function(window)
{
...
(function(window)
{
	var FWDR3DCovThumb = function(id, data, parent)
	{
		var self = this;
		...
		{
			self.mainDO = new FWDR3DCovDisplayObject("div");
			...
			{
				self.borderDO = new FWDR3DCovSimpleDisplayObject("div");
				self.bgDO = new FWDR3DCovSimpleDisplayObject("div");
			...
			self.imageHolderDO = new FWDR3DCovDisplayObject("div");
			...
			if (!self.isMobile && !FWDR3DCovUtils.isIEAndLessThen9 && data.showGradient)
			{
				self.setupGradient();
			}
		...
		{
			if (!self.imageDO || self.isMobile || FWDR3DCovUtils.isIEAndLessThen9)
				return;
			
			var imgW = self.thumbWidth - self.borderSize * 2;
			var imgH = self.thumbHeight - self.borderSize * 2;
			
			self.reflCanvasDO = new FWDR3DCovSimpleDisplayObject("canvas");
		...
		{
			self.imageDO = new FWDR3DCovSimpleDisplayObject("img");
		...
		{
			self.htmlContentDO = new FWDR3DCovSimpleDisplayObject("div");
		...
		this.setupGradient = function()
		{
			...
			self.gradientRightDO = new FWDR3DCovSimpleDisplayObject("img");
		...
		this.setGradient = function(pos)
		{
			...
			if (!self.isMobile && !FWDR3DCovUtils.isIEAndLessThen9 && data.showGradient)
			{
				...
				{
					case 0:
						FWDR3DCovModTweenMax.to(self.gradientLeftDO, .8, {alpha:0});
						FWDR3DCovModTweenMax.to(self.gradientRightDO, .8, {alpha:0, onComplete:self.setGradPos});
						break;
					case 1:
						self.gradientDO.setY(0);
						FWDR3DCovModTweenMax.to(self.gradientLeftDO, .8, {alpha:0});
						FWDR3DCovModTweenMax.to(self.gradientRightDO, .8, {alpha:1});
						break;
					case -1:
						self.gradientDO.setY(0);
						FWDR3DCovModTweenMax.to(self.gradientLeftDO, .8, {alpha:1});
						FWDR3DCovModTweenMax.to(self.gradientRightDO, .8, {alpha:0});
						break;
				}
			}
		};
		...
		this.showThumb3D = function()
		{
			...
			FWDR3DCovModTweenMax.to(self.imageHolderDO, .8, {x:self.borderSize, y:self.borderSize, w:imgW, h:imgH, ease:Expo.easeInOut});
			...
			if (data.showThumbnailsHtmlContent)
			{
				...
				FWDR3DCovModTweenMax.to(self.htmlContentDO, .8, {x:0, y:0, ease:Expo.easeInOut});
			}
			else
			{
				...
				FWDR3DCovModTweenMax.to(self.imageDO, .8, {x:0, y:0, ease:Expo.easeInOut});
				
				if (self.reflCanvasDO)
				{
					self.reflCanvasDO.setAlpha(0);
					FWDR3DCovModTweenMax.to(self.reflCanvasDO, .8, {alpha:1, ease:Expo.easeInOut});
				}
			}
		};
		...
		this.showThumb2D = function()
		{
			if (!FWDR3DCovUtils.hasTransform2d)
			{
				...
				FWDR3DCovModTweenMax.to(self.imageHolderDO, .8, {x:borderScale, y:borderScale, w:imgW, h:imgH, ease:Expo.easeInOut});
		...
		this.showThumbIntro2D = function(scale, del)
		{
			...
			if (!FWDR3DCovUtils.hasTransform2d)
			{
				...
				FWDR3DCovModTweenMax.to(self, .8, {x:Math.floor(self.newX + (self.thumbWidth - scaleW)/2), y:-Math.floor(scaleH/2), delay:del, ease:Expo.easeOut});
			}
			else
			{
				self.setScale2(self.thumbScale);
				...
				FWDR3DCovModTweenMax.to(self, .8, {x:self.newX, scale:self.thumbScale, delay:del, ease:Quart.easeOut, onComplete:self.setThumbVisibility});
			}
		};
		...
		this.setScale = function(scale, alpha)
		{
			...
			if (!FWDR3DCovUtils.hasTransform2d)
			{
				...
				if (self.borderDO)
				{
					FWDR3DCovModTweenMax.to(self.borderDO, .8, {w:scaleW, h:scaleH, ease:Quart.easeOut});
				}
				
				if (self.bgDO)
				{
					FWDR3DCovModTweenMax.to(self.bgDO, .8, {x:borderScale, y:borderScale, w:scaleW - borderScale * 2, h:scaleH - borderScale * 2, ease:Quart.easeOut});
				}
				
				FWDR3DCovModTweenMax.to(self.mainDO, .8, {w:scaleW, h:scaleH, ease:Quart.easeOut});
				FWDR3DCovModTweenMax.to(self.imageHolderDO, .8, {x:borderScale, y:borderScale, w:scaleW - borderScale * 2, h:scaleH - borderScale * 2, ease:Quart.easeOut});
				FWDR3DCovModTweenMax.to(self, .8, {x:Math.floor(self.newX + (self.thumbWidth - scaleW)/2), y:-Math.floor(scaleH/2), w:scaleW, h:scaleH, alpha:alpha, ease:Quart.easeOut});
				
				if (data.showThumbnailsHtmlContent)
				{
					if (self.htmlContentDO)
					{
						FWDR3DCovModTweenMax.to(self.htmlContentDO, .8, {w:scaleW - borderScale * 2, h:scaleH - borderScale * 2, ease:Quart.easeOut});
					}
				}
				else
				{
					if (self.imageDO)
					{
						FWDR3DCovModTweenMax.to(self.imageDO, .8, {w:scaleW - borderScale * 2, h:scaleH - borderScale * 2, ease:Quart.easeOut});
					}
				}
			}
			else
			{
				FWDR3DCovModTweenMax.to(self, .8, {x:Math.floor(self.newX), scale:self.thumbScale, alpha:alpha, ease:Quart.easeOut, onComplete:self.setThumbVisibility});
			}
		};
		...
		this.hide = function(del)
		{
			...
			FWDR3DCovModTweenMax.to(self.imageHolderDO, .8, {x:parseInt(self.thumbWidth/2), y:parseInt(self.thumbHeight/2), w:0, h:0, delay:del, ease:Expo.easeInOut});
			
			if (data.showThumbnailsHtmlContent)
			{
				if (self.htmlContentDO)
				{
					FWDR3DCovModTweenMax.to(self.htmlContentDO, .8, {x:-parseInt(imgW/2), y:-parseInt(imgH/2), delay:del, ease:Expo.easeInOut});
				}
			}
			else
			{
				if (self.imageDO)
				{
					FWDR3DCovModTweenMax.to(self.imageDO, .8, {x:-parseInt(imgW/2), y:-parseInt(imgH/2), delay:del, ease:Expo.easeInOut});
					
					if (self.reflCanvasDO)
					{
						FWDR3DCovModTweenMax.to(self.reflCanvasDO, .8, {alpha:0, delay:del, ease:Expo.easeInOut});
					}
				}
			}
		};
		...
		{
			self.dispatchEvent(FWDR3DCovThumb.CLICK, {id:self.id});
		};
		...
		this.onMouseOverHandler = function()
		{
			if (!self.isOver2 && (self.id != parent.curId))
			{
				...
				FWDR3DCovModTweenMax.to(self, .6, {z:self.curZ + 100, ease:Expo.easeOut});
			}
		};
		...
		this.onMouseOutHandler = function()
		{
			if (self.isOver2 && (self.id != parent.curId))
			{
				...
				if (self.gradientDO)
				{
					self.setGradient2(self.pos);
				}
				
				FWDR3DCovModTweenMax.to(self, .6, {z:self.curZ, ease:Expo.easeOut});
			}
		};
		...
		{
			FWDR3DCovModTweenMax.killTweensOf(self);
			FWDR3DCovModTweenMax.killTweensOf(self.mainDO);
			FWDR3DCovModTweenMax.killTweensOf(self.imageHolderDO);
			...
			if (self.imageDO)
			{
				FWDR3DCovModTweenMax.killTweensOf(self.imageDO);
				self.imageDO.destroy();
			}
			
			if (self.htmlContentDO)
			{
				FWDR3DCovModTweenMax.killTweensOf(self.htmlContentDO);
			...
			if (self.bgDO)
			{
				FWDR3DCovModTweenMax.killTweensOf(self.bgDO);
				self.bgDO.destroy();
				self.bgDO = null;
			}
			...
			if (self.borderDO)
			{
				FWDR3DCovModTweenMax.killTweensOf(self.borderDO);
			...
			{
				FWDR3DCovModTweenMax.killTweensOf(self.gradientLeftDO);
				self.gradientLeftDO.destroy();
			}
			
			if (self.gradientRightDO)
			{
				FWDR3DCovModTweenMax.killTweensOf(self.gradientRightDO);
	...
	{
		FWDR3DCovThumb.prototype = new FWDR3DCovDisplayObject3D("div", "absolute", "visible");
	};
...
(function (window){
	...
	FWDR3DCovUtils.trimAndFormatUrl = function(str){
	...
	/*DOM manipulation */
	//#############################################//
	FWDR3DCovUtils.parent = function (e, n){
		if(n === undefined) n = 1;
	...
	FWDR3DCovUtils.insertNodeAt = function(parent, child, n){
		var children = FWDR3DCovUtils.children(parent);
	...
	FWDR3DCovUtils.isMobile = (function (){
		if(FWDR3DCovUtils.hasPointerEvent && navigator.msMaxTouchPoints > 1) return true;
	...
	FWDR3DCovUtils.isIEAndMoreThen8 = (function(){
		return FWDR3DCovUtils.isIE9 || FWDR3DCovUtils.isIE10 || FWDR3DCovUtils.isIE11;
	}());
	...
	FWDR3DCovUtils.isIE9 = (function(){
		return navigator.userAgent.toLowerCase().indexOf("msie 9") != -1;
	}());
	
	FWDR3DCovUtils.isIE10 = (function(){
	...
	FWDR3DCovUtils.isIEAndMoreThen9 = (function(){
		return FWDR3DCovUtils.isIE10 || FWDR3DCovUtils.isIE11;
	}());
	
	FWDR3DCovUtils.isApple = (function(){
	...
	{
		if (FWDR3DCovUtils.isReadyMethodCalled_bl)
			return;
	...
	FWDR3DCovUtils.getUrlArgs = function urlArgs(string){
		var args = {};
	...
	FWDR3DCovUtils.validateEmail = function(mail){  
	...
	/* resize utils */
	//################################//
	FWDR3DCovUtils.resizeDoWithLimit = function(
			displayObject,
			containerWidth,
			containerHeight,
			doWidth,
		...
		if(animate){
			FWDR3DCovModTweenMax.to(displayObject, pDuration, {x:x, y:y, w:finalWidth, h:finalHeight, delay:pDelay, ease:pEase});
		}else{
			displayObject.x = x;
			displayObject.y = y;
...
(function(window)
{
	var FWDRoyal3DCoverflow = function(props)
	{
		var self = this;
		...
		this.init = function()
		{
			...
			if (!FWDR3DCovUtils.getChildById(self.propsObj.coverflowHolderDivId))
			{
				alert("FWDRoyal3DCoverflow holder div is not found, please make sure that the div exists and the id is correct! " + self.propsObj.coverflowHolderDivId);
				return;
			}
			...
			self.stageContainer = FWDR3DCovUtils.getChildById(self.propsObj.coverflowHolderDivId);
		...
		{
			self.mainDO = new FWDR3DCovDisplayObject("div", "relative");
			...
			if (self.displayType == FWDRoyal3DCoverflow.FLUID_WIDTH)
			{	
		...
		this.setBackgrounds = function()
		{
			...
			{
				self.bgDO = new FWDR3DCovDisplayObject("div");
				self.mainDO.addChild(self.bgDO);
				...
				FWDR3DCovModTweenMax.to(self.bgDO, .8, {alpha:1});
			}

			if (self.propsObj.thumbnailsBackgroundImagePath)
			{
				self.thumbsBgDO = new FWDR3DCovDisplayObject("div");
				...
				FWDR3DCovModTweenMax.to(self.thumbsBgDO, .8, {alpha:1});
			}

			if (self.propsObj.scrollbarBackgroundImagePath)
			{
				self.scrollbarBgDO = new FWDR3DCovDisplayObject("div");
				...
				FWDR3DCovModTweenMax.to(self.scrollbarBgDO, .8, {alpha:1});
			}
		};
		...
		this.resizeHandler = function()
		{
			...
			if (self.preloaderLoaded)
			{
				if (self.propsObj.backgroundImagePath)
				{
					if (self.displayType == FWDRoyal3DCoverflow.RESPONSIVE)
					{
		...
		this.setupContextMenu = function()
		{
			self.customContextMenuDO = new FWDR3DCovContextMenu(self.mainDO, self.data.showContextMenu);
		};
		...
		this.setupData = function()
		{
			...
			self.data = new FWDR3DCovData(self.propsObj);
			self.data.addListener(FWDR3DCovData.PRELOADER_LOAD_DONE, self.onPreloaderLoadDone);
			self.data.addListener(FWDR3DCovData.LOAD_ERROR, self.dataLoadError);
			self.data.addListener(FWDR3DCovData.LOAD_DONE, self.dataLoadComplete);
		};
		...
		{
			self.dispatchEvent(FWDRoyal3DCoverflow.DATA_LOADED);
		...
		this.setupPreloader = function()
		{
			...
			self.preloaderDO = new FWDR3DCovPreloader(self.data.mainPreloaderImg, 85, 36, 20, 50);
		...
		this.setupThumbsManager = function()
		{
			...
			self.thumbsManagerDO = new FWDR3DCovThumbsManager(self.data, self);
			self.thumbsManagerDO.addListener(FWDR3DCovThumbsManager.THUMB_CLICK, self.onThumbsManagerThumbClick);
			self.thumbsManagerDO.addListener(FWDR3DCovThumbsManager.LOAD_ERROR, self.onThumbsManagerLoadError);
			self.thumbsManagerDO.addListener(FWDR3DCovThumbsManager.THUMBS_INTRO_FINISH, self.onThumbsManagerIntroFinish);
			self.thumbsManagerDO.addListener(FWDR3DCovThumbsManager.THUMB_CHANGE, self.onThumbsManagerThumbChange);
		...
		{
			self.enableAll();
			self.dispatchEvent(FWDRoyal3DCoverflow.INTRO_FINISH);
			
			self.apiReady = true;
			self.dispatchEvent(FWDRoyal3DCoverflow.IS_API_READY);
			
			self.dispatchEvent(FWDRoyal3DCoverflow.CATEGORY_CHANGE, {id:self.thumbsManagerDO.dataListId});
		};
		
		this.onThumbsManagerThumbChange = function(e)
		{
			self.dispatchEvent(FWDRoyal3DCoverflow.THUMB_CHANGE, {id:e.id});
		};
		...
		this.setupComboBox = function()
		{
			...
			self.comboBoxDO = new FWDR3DCovComboBox(self, 
			{
			...
			self.comboBoxDO.addListener(FWDR3DCovComboBox.BUTTON_PRESSED, self.onComboboxButtonPressHandler);
		...
		this.onComboboxButtonPressHandler = function(e)
		{
			if (self.thumbsManagerDO.allowToSwitchCat)
			{
				...
				self.dispatchEvent(FWDRoyal3DCoverflow.INTRO_START);
		...
		this.setupLightBox = function()
		{
			...
			this.lightboxDO = new FWDR3DCovLightBox(
			{
				//main data array
		...
		{
			self.disableDO = new FWDR3DCovDisplayObject3D("div");
	   ...
	   this.removeListener = function(type, listener)
	   {
		...
		this.destroy = function()
		{
			...
			if (self.thumbsBgDO)
			{
				FWDR3DCovModTweenMax.killTweensOf(self.thumbsBgDO);
				self.thumbsBgDO.destroy();
			}
			
			if (self.scrollbarBgDO)
			{
				FWDR3DCovModTweenMax.killTweensOf(self.scrollbarBgDO);
	...
	FWDRoyal3DCoverflow.RESIZE = "resize";
	FWDRoyal3DCoverflow.LIGHTBOX = "lightbox";
	FWDRoyal3DCoverflow.RESPONSIVE = "responsive";
	FWDRoyal3DCoverflow.FLUID_WIDTH = "fluidwidth";
	
	FWDRoyal3DCoverflow.INTRO_START = "onsIntroStart";
	FWDRoyal3DCoverflow.INTRO_FINISH = "onsIntroFinish";
	FWDRoyal3DCoverflow.DATA_LOADED = "onDataLoaded";
	FWDRoyal3DCoverflow.IS_API_READY = "isAPIReady";
	FWDRoyal3DCoverflow.CATEGORY_CHANGE = "categoryChange";
	FWDRoyal3DCoverflow.THUMB_CHANGE = "thumbChanged";
	
	window.FWDRoyal3DCoverflow = FWDRoyal3DCoverflow;
